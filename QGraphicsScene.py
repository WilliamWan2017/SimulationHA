...
class LineItem(QGraphicsLineItem):
    def __init__(self,   x1, y1, x2, y2,scene,  style=Qt.SolidLine,
                 rect=None, matrix=QTransform()):
...
class MainForm(QDialog):
    ...
    def addLine(self): 
        LineItem(0, 150, 200, 350, self.scene)
        self.scene.update()
    ...
    def readItemFromStream(self, stream, offset=0):
        ...
        if type == "Text":
            ...
            style = Qt.PenStyle(stream.readInt16())
            rotateangle=stream.readFloat()
            ln=LineItem(x1, y1, x2, y2, self.scene)
    ...
    def writeItemToStream(self, stream, item):
        if isinstance(item, TextItem):
            ...
            stream.writeFloat( rect.bottomRight().x())
            stream.writeFloat( rect.bottomRight().y())
            stream.writeInt16(item.style)
